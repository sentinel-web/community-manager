...
`/test`, `/verify`, `/review`, `/validate`, `/pr`).
...
10. **Verify in the UI** - Run `/verify` to exercise the affected golden path in a browser (types and tests prove correctness, not feature behavior)
11. **Run Code Review** - Self-review or request peer review
12. **Fix Review Issues** - Address feedback from review
13. **Validate Against CLAUDE.md** - Ensure code follows documented patterns; update CLAUDE.md if new patterns emerge
14. **Create Pull Request** - Include summary of changes and steps for testing/reproduction
