...
- Chrome: `app/`, `main/`, `header/`, `footer/`, `navigation/`, `login/`, `theme/`, `title/`, `logo/`, `profile-picture-input/`, `suspense/`, `section/`, `table/`, `components/`
...
**Mutation Pipeline** (`server/mutation-pipeline.ts`) — the canonical mutation primitive
- `runMutation(ctx, descriptor, args, body)` owns the full lifecycle: auth → permission (with `fallbackFlag` + `permissionOverride` re-admit) → input validation → body → audit log. Denial paths emit a `<collection>.<op>.denied` audit entry capturing `userId` (and `id` for `update`/`delete`) before throwing — there is no permission-denied path that skips audit.
- The `createCollectionMethods` factory in `server/crud.lib.ts` routes every standard CRUD method through `runMutation` automatically; you only need to call `runMutation` directly for *custom* methods that want the same lifecycle.
- Body errors propagate unwrapped: a `Meteor.Error('SomeCode', '...')` thrown inside `body` reaches the caller with `error.error === 'SomeCode'` intact.
- See `CONTEXT.md` → "MutationWithAudit" for the descriptor field reference and the audit-shape contract.

**Custom Meteor Methods** (when not generated by `createCollectionMethods`)
- Prefer `runMutation` with a descriptor over hand-rolled checks — you get auth, permission, audit-on-denial, and validation in one place.
- Hand-rolled fallback (only when the pipeline doesn't fit): `async` functions with `*Async` collection methods (`findOneAsync`, `insertAsync`, `updateAsync`, `removeAsync`); `validateString()`/`validateObject()`/etc. on inputs; `if (!this.userId) throw new Meteor.Error(401, 'Unauthorized')`; `await checkPermission(this.userId, module, operation)`; `Meteor.Error(code, message)` (never generic `Error`); `createLog(action, data)` for audit. If you find yourself reaching for the hand-rolled path, it's almost certainly a sign to add the missing knob to `MutationDescriptor` instead.
...
Two skill conventions coexist in `.claude/skills/`:
- **Project skills** — single `.md` files (e.g. `issue.md`, `collection.md`). Bespoke automations tailored to this codebase.
- **Engineering skills** — directories with `SKILL.md` + bundled resources (e.g. `diagnose/`, `tdd/`). Installed via `setup-matt-pocock-skills` and configured by the `## Agent skills` section below (`docs/agents/issue-tracker.md`, `docs/agents/triage-labels.md`, `docs/agents/domain.md`).
...
**Engineering Skills** (directory-style, configured via `docs/agents/`)
| Skill | Purpose |
|-------|---------|
| `/diagnose` | Disciplined reproduce → minimise → hypothesise → instrument → fix → regression-test loop for hard bugs |
| `/tdd` | Red-green-refactor loop for feature work and bug fixes |
| `/triage` | Move issues through the canonical triage state machine |
| `/to-issues` | Break a plan or PRD into tracer-bullet GitHub issues |
| `/to-prd` | Turn the current conversation into a PRD on the issue tracker |
| `/grill-me` | Interview the user to stress-test a plan or design |
| `/grill-with-docs` | Stress-test a plan against `CONTEXT.md` and `docs/adr/`, updating docs inline |
| `/improve-codebase-architecture` | Find deepening / consolidation opportunities informed by domain docs |
| `/zoom-out` | Step back from local context to a higher-level perspective on a code area |
| `/write-a-skill` | Author a new directory-style skill with progressive disclosure |
| `/caveman` | Ultra-compressed communication mode |
| `/setup-matt-pocock-skills` | (Re)wire `docs/agents/` and the `## Agent skills` block for this repo |
...
`npm run doctor` runs react-doctor over the codebase (perf / hooks / correctness / architecture / a11y). Known false positives — including the React 19 rule firing on this React 18 codebase, knip flagging Meteor entry points as unused, and Playwright's `use` fixture tripping `rules-of-hooks` — are catalogued in `docs/agents/doctor.md`. Read that doc before chasing findings; it also lists the highest-leverage categories to actually fix (`async-await-in-loop`, `async-parallel`, `server-sequential-independent-await`).
